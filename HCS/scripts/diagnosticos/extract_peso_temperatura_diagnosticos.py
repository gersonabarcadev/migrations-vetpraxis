...
# -*- coding: utf-8 -*-
"""
Script para extraer PESO y TEMPERATURA del campo NOTE en diagnósticos
Adaptado del script de extracción de procedimientos
"""
...
def process_diagnosticos_with_peso_temp(input_file=None, output_dir=None):
    """Procesa los datos limpios de diagnosticos extrayendo peso y temperatura"""
    
    # Configurar rutas por defecto si no se proporcionan
    if input_file is None:
        base_path = os.path.dirname(os.path.dirname(__file__))
        input_file = os.path.join(base_path, "generation", "diagnosticos_organized.xlsx")
    else:
        # SIEMPRE buscar el archivo organized en el output_dir cuando se ejecuta desde master script
        if output_dir:
            organized_file = os.path.join(output_dir, "diagnosticos_organized.xlsx")
            if os.path.exists(organized_file):
                input_file = organized_file
                print(f"[OK] Usando archivo organized: {os.path.basename(input_file)}")
            else:
                print(f"[X] ERROR: No se encontró diagnosticos_organized.xlsx en {output_dir}")
                return None
    ...
    output_file = os.path.join(output_dir, "diagnosticos_with_peso_temp.xlsx")
    
    print("[LAB] EXTRAYENDO PESO Y TEMPERATURA DE DIAGNÓSTICOS")
    print("="*60)
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        df_clean.to_excel(writer, sheet_name='Diagnosticos_Con_Peso_Temp', index=False)
...
def main():
    ...
    print("[>>] EXTRAYENDO PESO Y TEMPERATURA DE DIAGNÓSTICOS")
    ...
    try:
        result = process_diagnosticos_with_peso_temp(input_file, output_dir)
        if result is not None:
            print(f"\n[OK] Proceso completado exitosamente")
            print(f"[DATA] Datos enriquecidos con información fisiológica extraída")
        else:
            print(f"\n[ERROR] El proceso falló. Revisa los errores anteriores.")
