...
def transform_to_import(input_file=None, output_dir=None):
    """
    Transforma los datos de diagnósticos al formato de importación NOTAS
    """
    
    # Configurar rutas por defecto si no se proporcionan
    if input_file is None:
        source_file = "../generation/diagnosticos_with_peso_temp.xlsx"
    else:
        # Si se proporciona input_file, buscar el archivo with_peso_temp en el output_dir
        if output_dir and os.path.exists(os.path.join(output_dir, "diagnosticos_with_peso_temp.xlsx")):
            source_file = os.path.join(output_dir, "diagnosticos_with_peso_temp.xlsx")
        else:
            source_file = "../generation/diagnosticos_with_peso_temp.xlsx"
    ...
    output_file = os.path.join(output_dir, f"diagnosticos_import_transformed.xlsx")
    
    print("=" * 80)
    print("TRANSFORMACIÓN DE DIAGNÓSTICOS A FORMATO DE IMPORTACIÓN NOTAS")
    print("=" * 80)
    ...
    try:
        ...
        # Cargar datos fuente
        source_df = pd.read_excel(source_file, sheet_name='Diagnosticos_Con_Peso_Temp')
        ...
        # Debe tener PatientDiagnosticId, PatientId y DataDate
        required_fields = ['PatientDiagnosticId', 'PatientId', 'DataDate']
        ...
        # Verificar duplicados por PatientDiagnosticId
        if 'PatientDiagnosticId' in source_df.columns:
            duplicates = source_df['PatientDiagnosticId'].duplicated().sum()
            if duplicates > 0:
                add_to_report(f"ADVERTENCIA: {duplicates} PatientDiagnosticId duplicados encontrados")
                source_df = source_df.drop_duplicates(subset=['PatientDiagnosticId'], keep='first')
        ...
        # 1. ID ATENCION <- PatientDiagnosticId
        df_transformed['ID ATENCION'] = source_df['PatientDiagnosticId'] if 'PatientDiagnosticId' in source_df.columns else None
        add_to_report("  - PatientDiagnosticId -> ID ATENCION")
        ...
        if 'FECHA' in df_transformed.columns and df_transformed['FECHA'].notna().sum() > 0:
            ...
            year_stats = df_transformed.groupby('año').agg({
                ...
                'ID ATENCION': 'total_diagnosticos',
                'ID MASCOTA': 'mascotas_unicas'
            })
            
            for año, stats in year_stats.iterrows():
                add_to_report(f"Año {año}: {stats['total_diagnosticos']} diagnósticos, {stats['mascotas_unicas']} mascotas únicas")
        ...
        if 'IsDeleted' in pd.read_excel(source_file, sheet_name='Diagnosticos_Con_Peso_Temp', nrows=1).columns:
            df_all = pd.read_excel(source_file, sheet_name='Diagnosticos_Con_Peso_Temp')
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            mapeo_docs = {
                ...
                'Descripcion': [
                    'ID único del diagnóstico del paciente',
                    'ID único del paciente/mascota',
                    'Fecha del diagnóstico',
                    'Concatenación de nombre, notas y descripción del diagnóstico'
                ]
            }
...
def main():
    ...
    print("[>>] TRANSFORMANDO DIAGNÓSTICOS AL FORMATO NOTAS")
