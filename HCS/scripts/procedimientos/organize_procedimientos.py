...
# -*- coding: utf-8 -*-
"""
Script mejorado para organizar los datos de procedimientos en hojas separadas
...
def organize_procedimientos_data(input_file=None, output_dir=None):
    """Organiza los datos de procedimientos en hojas separadas por estado"""
    
    # Configurar rutas por defecto si no se proporcionan
    if input_file is None:
        base_path = os.path.dirname(os.path.dirname(__file__))
        input_file = os.path.join(base_path, "generation", "procedimientos_merged.xlsx")
    else:
        # SIEMPRE buscar el archivo merged en el output_dir cuando se ejecuta desde master script
        if output_dir:
            merged_file = os.path.join(output_dir, "procedimientos_merged.xlsx")
            if os.path.exists(merged_file):
                input_file = merged_file
                print(f"[OK] Usando archivo merged: {os.path.basename(input_file)}")
            else:
                print(f"[X] ERROR: No se encontró procedimientos_merged.xlsx en {output_dir}")
                return None
    ...
    output_file = os.path.join(output_dir, "procedimientos_organized.xlsx")
    
    print("[DATA] ORGANIZANDO DATOS DE PROCEDIMIENTOS EN HOJAS SEPARADAS")
    print("="*60)
    print(f"[DIR] Archivo origen: {os.path.basename(input_file)}")
    
    # Cargar el archivo merged
    df_all = pd.read_excel(input_file, sheet_name='Procedimientos_Merged')
    ...
    # 2. SIN MATCH (registros que no encontraron procedimiento)
    print(f"\n[X] HOJA 2 - SIN MATCH:")
    # Los que no tienen match son los que no tienen nombre del procedimiento
    df_no_match = df_all[df_all['Name'].isna()].copy()
    print(f"   - Registros sin match: {len(df_no_match):,}")
    
    if len(df_no_match) > 0:
        missing_ids = df_no_match['InterventionId'].unique()
        print(f"   - IDs de procedimientos faltantes: {list(missing_ids)}")
        
        # Mostrar detalles de los registros sin match
        for idx, row in df_no_match.iterrows():
            print(f"     * Paciente {row['PatientId']}, InterventionId {row['InterventionId']}, Fecha {row['DataDate'].strftime('%Y-%m-%d')}")
    ...
    if len(df_deleted) > 0:
        deleted_procedures = df_deleted['Name'].value_counts().head(5)
        print(f"   - Top procedimientos eliminados:")
        for proc, count in deleted_procedures.items():
            if pd.notna(proc):
                print(f"     * {proc}: {count} registros")
    
    # 4. DATOS LIMPIOS (sin eliminados y con match)
    print(f"\n[CLEAN] HOJA 4 - DATOS LIMPIOS:")
    ...
    print(f"   - Procedimientos únicos: {df_clean['InterventionId'].nunique():,}")
    ...
    # TOP PROCEDIMIENTOS EN DATOS LIMPIOS
    if len(df_clean) > 0 and 'Name' in df_clean.columns:
        print(f"\n[TOP] TOP 5 PROCEDIMIENTOS EN DATOS LIMPIOS:")
        top_clean = df_clean['Name'].value_counts().head(5)
        for i, (proc, count) in enumerate(top_clean.items(), 1):
            if pd.notna(proc):
                print(f"   {i}. {proc}: {count} veces")
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        stats_data = {
            ...
            'Cantidad': [
                ...
                df_clean['InterventionId'].nunique() if len(df_clean) > 0 else 0,
    ...
    print(f"   [CLEAN] Hoja 4: Datos limpios ({len(df_clean):,})")
...
def main():
    ...
    print("[>>] ORGANIZANDO DATOS DE PROCEDIMIENTOS")
    ...
    try:
        result = organize_procedimientos_data(input_file, output_dir)
