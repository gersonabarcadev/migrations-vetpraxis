...
def transform_to_import(input_file=None, output_dir=None):
    """
    Transforma los datos de vacunas al formato de importación NOTAS
    """
    
    # Rutas de archivos - usar parámetros o valores por defecto
    if input_file and output_dir:
        source_file = os.path.join(output_dir, "vacunas_with_peso_temp.xlsx")
        target_dir = output_dir
    else:
        source_file = "../generation/vacunas_with_peso_temp.xlsx"
        target_dir = "../generation"
    
    # Crear directorio de salida si no existe
    os.makedirs(target_dir, exist_ok=True)
    
    output_file = os.path.join(target_dir, f"vacunas_import_transformed.xlsx")
    
    print("=" * 80)
    print("TRANSFORMACIÓN DE VACUNAS A FORMATO DE IMPORTACIÓN NOTAS")
    print("=" * 80)
    ...
    try:
        ...
        # Cargar datos fuente
        source_df = pd.read_excel(source_file, sheet_name='Vacunas_Con_Peso_Temp')
        ...
        # Debe tener PatientVaccineId, PatientId y DataDate
        required_fields = ['PatientVaccineId', 'PatientId', 'DataDate']
        ...
        # Verificar duplicados por PatientVaccineId
        if 'PatientVaccineId' in source_df.columns:
            duplicates = source_df['PatientVaccineId'].duplicated().sum()
            if duplicates > 0:
                add_to_report(f"ADVERTENCIA: {duplicates} PatientVaccineId duplicados encontrados")
                source_df = source_df.drop_duplicates(subset=['PatientVaccineId'], keep='first')
        ...
        # 1. ID ATENCION <- PatientVaccineId
        df_transformed['ID ATENCION'] = source_df['PatientVaccineId'] if 'PatientVaccineId' in source_df.columns else None
        add_to_report("  - PatientVaccineId -> ID ATENCION")
        ...
        if 'FECHA' in df_transformed.columns and df_transformed['FECHA'].notna().sum() > 0:
            ...
            year_stats = df_transformed.groupby('año').agg({
                ...
                'ID ATENCION': 'total_vacunas',
                'ID MASCOTA': 'mascotas_unicas'
            })
            
            for año, stats in year_stats.iterrows():
                add_to_report(f"Año {año}: {stats['total_vacunas']} vacunas, {stats['mascotas_unicas']} mascotas únicas")
        ...
        if 'IsDeleted' in pd.read_excel(source_file, sheet_name='Vacunas_Con_Peso_Temp', nrows=1).columns:
            df_all = pd.read_excel(source_file, sheet_name='Vacunas_Con_Peso_Temp')
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            mapeo_docs = {
                ...
                'Descripcion': [
                    'ID único de la vacunación del paciente',
                    'ID único del paciente/mascota',
                    'Fecha de la vacunación',
                    'Concatenación de nombre, notas y descripción de la vacuna'
                ]
            }
...
def main():
    ...
    print("[>>] TRANSFORMANDO VACUNAS A FORMATO NOTAS")
    ...
    try:
        ...
        if df_result is not None:
            validate_result = validate_output(output_path)
            if validate_result:
                print(f"\n[OK] Transformación completada exitosamente")
                print(f"[FILE] Archivo generado: {output_path}")
            else:
                print(f"\n[WARN] Transformación completada pero con advertencias")
        else:
