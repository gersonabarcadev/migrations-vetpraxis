#!/usr/bin/env python3
"""
Generador de templates Excel para importación de procedimientos
Nueva estructura específica con IDs correlativos

Formato de salida:
A => import_clinic_record_id (correlativo desde 99834681)
B => PatientId 
C => DataDate (fecha de aplicación)
D => Note (procedimientos.Name + [PARRAFO] + pacienteprocedimientos.Note)
"""
...
def load_and_prepare_data():
    """Cargar y preparar datos de procedimientos"""
    print("📖 Cargando datos de procedimientos...")
    ...
    print(f"✅ Catálogo de procedimientos: {len(df_procedimientos):,} registros")
    print(f"✅ Aplicaciones de procedimientos: {len(df_pacienteprocedimientos):,} registros")
    
    # Filtrar solo registros activos (no eliminados)
    if 'IsDeleted' in df_pacienteprocedimientos.columns:
        df_active = df_pacienteprocedimientos[df_pacienteprocedimientos['IsDeleted'] == 0].copy()
        eliminados = len(df_pacienteprocedimientos) - len(df_active)
        print(f"🗑️  Eliminados: {eliminados:,} registros ({eliminados/len(df_pacienteprocedimientos)*100:.1f}%)")
    else:
        df_active = df_pacienteprocedimientos.copy()
    ...
    # Crear diccionario de nombres de procedimientos
    procedure_names = dict(zip(df_procedimientos['InterventionId'], df_procedimientos['Name']))
    
    print(f"🔗 Catálogo de procedimientos disponible: {len(procedure_names)} tipos")
    
    return df_active, procedure_names

def process_procedure_records(df_active, procedure_names):
    """Procesar registros de procedimientos para importación"""
    print("\n🔄 Procesando registros de procedimientos...")
    
    records = []
    current_id = 99834681  # ID inicial correlativo
    ...
    for idx, row in df_sorted.iterrows():
        try:
            ...
            intervention_id = int(row['InterventionId'])
            note = row.get('Note', '') if pd.notna(row.get('Note', '')) else ''
            
            # Obtener nombre del procedimiento
            procedure_name = procedure_names.get(intervention_id, f"Procedimiento ID {intervention_id}")
            
            # Construir campo Note según especificación
            if note.strip():
                # Hay nota: Nombre + [PARRAFO] + Nota
                formatted_note = f"{procedure_name}[PARRAFO]{note.strip()}"
            else:
                # Sin nota: Solo nombre
                formatted_note = procedure_name
    ...
    print(f"🆔 IDs asignados: {99834681} - {current_id - 1}")
...
def create_excel_template(df_data, output_dir):
    ...
    for file_num in range(num_files):
        ...
        # Nombre del archivo
        filename = f"procedimientos_import_{file_num + 1:02d}.xlsx"
        filepath = os.path.join(output_dir, filename)
        
        # Crear workbook
        wb = Workbook()
        ws = wb.active
        ws.title = "Procedimientos Import"
...
def generate_summary_report(df_data, output_dir):
    ...
    summary_file = os.path.join(output_dir, "resumen_procedimientos_import.txt")
    
    with open(summary_file, 'w', encoding='utf-8') as f:
        f.write("RESUMEN DE IMPORTACIÓN - PROCEDIMIENTOS\n")
        f.write("=" * 50 + "\n\n")
        ...
        for year, count in year_counts.items():
            f.write(f"  {year}: {count:,} procedimientos\n")
        ...
        f.write(f"  Solo nombre del procedimiento: {notes_only_name:,} ({notes_only_name/len(df_data)*100:.1f}%)\n")
        f.write(f"  Con notas adicionales: {notes_with_paragraph:,} ({notes_with_paragraph/len(df_data)*100:.1f}%)\n")
        
        # Top procedimientos
        # Extraer nombres de procedimientos de las notas
        procedure_names = []
        for note in df_data['Note']:
            if '[PARRAFO]' in note:
                procedure_name = note.split('[PARRAFO]')[0]
            else:
                procedure_name = note
            procedure_names.append(procedure_name)
        
        f.write(f"\nTOP 10 PROCEDIMIENTOS MÁS FRECUENTES:\n")
        procedure_counts = pd.Series(procedure_names).value_counts().head(10)
        for procedure, count in procedure_counts.items():
            f.write(f"  {procedure}: {count:,} aplicaciones\n")
        
        # Estadísticas de pacientes
        f.write(f"\nESTADÍSTICAS DE PACIENTES:\n")
        f.write(f"  Pacientes únicos: {df_data['PatientId'].nunique():,}\n")
        f.write(f"  Promedio procedimientos por paciente: {len(df_data)/df_data['PatientId'].nunique():.2f}\n")
        
        f.write(f"\nFORMATO DE ARCHIVOS:\n")
        f.write(f"  Columna A: Import Clinic Record ID (correlativo desde 99834681)\n")
        f.write(f"  Columna B: Import Pet ID (PatientId original)\n")
        f.write(f"  Columna C: Import Date (fecha de aplicación)\n")
        f.write(f"  Columna D: Note (Nombre[PARRAFO]Nota o solo Nombre)\n")
        f.write(f"\nSEPARADOR:\n")
        f.write(f"  [PARRAFO] - Para separar nombre del procedimiento de notas adicionales\n")
...
def generate_validation_samples(df_data, output_dir):
    ...
    with open(samples_file, 'w', encoding='utf-8') as f:
        f.write("MUESTRAS DE VALIDACIÓN - PROCEDIMIENTOS\n")
        f.write("=" * 50 + "\n\n")
        ...
        f.write("EJEMPLOS SOLO CON NOMBRE DE PROCEDIMIENTO:\n")
        f.write("-" * 40 + "\n")
...
def main():
    print("🏥 GENERADOR DE TEMPLATES - PROCEDIMIENTOS")
    print("=" * 50)
    
    output_dir = "/Users/enrique/Proyectos/imports/generated_files/procedimientos"
    
    try:
        # 1. Cargar datos
        df_active, procedure_names = load_and_prepare_data()
        
        # 2. Procesar registros
        df_result = process_procedure_records(df_active, procedure_names)
        ...
        for filepath in generated_files:
            ...
            print(f"   • {filename}")
        print(f"   • resumen_procedimientos_import.txt")
